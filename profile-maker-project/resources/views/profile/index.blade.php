    ...
    <script type="text/javascript">
      ...
      $(() => {
        ...
        btn.addEventListener('click', function() {
          ...
          localStorage.setItem('name-furigana', $('#name-furigana').val());
          localStorage.setItem('age', $('#age').val());
          localStorage.setItem('route', $('#route').val());
          localStorage.setItem('near-station', $('#near-station').val());
          ...
          localStorage.setItem('gender', checkValue);

          localStorage.setItem('license-year-1', $('#license-year-1').val());
          localStorage.setItem('license-name-1', $('#license-name-1').val());
          localStorage.setItem('license-year-2', $('#license-year-2').val());
          localStorage.setItem('license-name-2', $('#license-name-2').val());
          localStorage.setItem('license-year-3', $('#license-year-3').val());
          localStorage.setItem('license-name-3', $('#license-name-3').val());
          localStorage.setItem('knowledge', $('#knowledge').val());
          localStorage.setItem('pr', $('#pr').val());

          // 経歴データをフォームの数だけ生成し配列かしてローカルストレージに保存
          let histories = [];
          Array.prototype.slice.call(document.getElementsByClassName('historyBlock')).forEach((element) => {
            const v = {
              year: element.querySelector('.year').value,
              kind: element.querySelector('.kind').value,
              value: element.querySelector('.value').value,
              tips: element.querySelector('.tips').value,
            };
            histories.push(v);
          });
          localStorage.setItem('histories', JSON.stringify(histories));

          window.location.href = `${path.join("/")}/profile.html`;
        })
      })
    </script>
  </body>
